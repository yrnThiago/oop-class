...
export function BestServiceBuyers() {
    ...
    const fetchClients = async () => {
      ...
      try {
        const [clientsResponse] = await Promise.all([
          apiService.get(`${apiEndpoint}/topServiceBuyer/10`),
    ...
    <Card className='max-h-[70vh] flex flex-col'>
      <CardHeader>
        <div className="flex justify-between">
          <CardTitle className='text-2xl'>Maiores Compradores de Serviços (Quantidade)</CardTitle>
          ...
          <TableBody>
            {clients.map((client, idx) => (
              <TableRow key={client.id}>
                ...
                <TableCell className="font-medium">{client.totalServicesBought} Produtos</TableCell>
              </TableRow>
            ))}
          </TableBody>
        </Table>
