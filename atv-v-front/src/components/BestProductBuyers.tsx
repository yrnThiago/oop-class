...
export function BestProductBuyers() {
    ...
    const fetchClients = async () => {
      ...
      try {
        const [clientsResponse] = await Promise.all([
          apiService.get(`${apiEndpoint}/topProductBuyer/10`),
    ...
    <Card className='max-h-[70vh] flex flex-col'>
      <CardHeader>
        <div className="flex justify-between">
          <CardTitle className='text-2xl'>Maiores Compradores de Produtos (Quantidade)</CardTitle>
          ...
          <TableBody>
            {clients.map((client, idx) => (
              <TableRow key={client.id}>
                ...
                <TableCell className="font-medium">{client.totalProductsBought} Produtos</TableCell>
              </TableRow>
            ))}
          </TableBody>
        </Table>
